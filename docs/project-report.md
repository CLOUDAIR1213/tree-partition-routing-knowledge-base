...
项目是可信本地环境下的演示 MVP，不是生产知识平台。当前已有单文档删除、改分区和重新审核，但没有认证、权限、角色审批、多租户、服务端会话、通用失败重试、后台任务、OCR、数据库迁移、限流、指标、追踪或生产部署方案。
...
生产 `IndexRegistry.verify` 已逐个验证全部 Chunk ID，删除和索引迁移还使用 `verify_absent` 确认旧分区无残留。真实大文档下的验证成本仍需观测。
...
`(document_id, chunk_index)` 唯一，`document_id` 是启用 SQLite 外键约束的级联外键。单文档删除 API 会先清理三个 txtai 分区和文件目录，再删除 `documents` 记录并依靠外键级联删除 Chunk。
...
4. 生产索引 verify 已逐个检查全部 Chunk，大文档下的验证性能仍需观测。
