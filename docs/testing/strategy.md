...
| 前端组件 | `frontend/src/App.test.tsx` | 页面路由、聊天、上传、审核、受限 Markdown、assistant 右上角计时，以及请求期间切换会话、新会话首问、跨会话并发、失败和重试归属 |
...
`FakeIndexRegistry` 记录 search/upsert/delete 调用，按分区保存测试行，并同时实现全 Chunk 存在和删除验证。`FakeLLMProvider` 返回测试内定义的结构化 JSON，不访问网络。所有上传、SQLite 和索引数据位于 pytest `tmp_path` 生命周期中。
...
| 实际内网监听与远端访问 | 已完成普通启动验证 | 服务监听 `0.0.0.0:8000`，局域网地址 health 与 SPA 路由均返回 `200`；用户确认远端验收完成。本轮未启用防火墙，不将其视为来源限制验证 |
...
- 生产 IndexRegistry 已逐个验证全部 Chunk，但尚无针对真实 txtai 的大规模索引迁移和删除性能测试。
